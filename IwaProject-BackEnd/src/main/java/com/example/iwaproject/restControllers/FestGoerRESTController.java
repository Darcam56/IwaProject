...
public class FestGoerRESTController {
    ...
    @DeleteMapping("/{username}/festivals/{festId}")
    public ResponseEntity<FestGoer> delFestivalFromFestGoer(@PathVariable("username") String username, @PathVariable("festId") long festId){
        if (username != null) {
            FestGoer festGoer = (FestGoer) userRepository.findByUsername(username)
                    .orElseThrow(() -> new RuntimeException("Fail -> Cause: User not found."));
            Festival fest = festivalRepository.findById(festId);

            festGoer.removeFestival(fest);
            userRepository.save(festGoer);
            return new ResponseEntity<>(HttpStatus.ACCEPTED);
        }
        return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
